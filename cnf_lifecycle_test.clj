;; cnf_lifecycle_test.clj — Stage 4 gate: the life-os lifecycle derivations
...
;;   CHELONIA_LOG=/path bb -cp out cnf_lifecycle_test.clj
(require '[chelonia.cnf :as c] '[chelonia.schema :as s] '[chelonia.datalog :as d]
         '[chelonia.kernel :as k] '[chelonia.projections :as proj] '[chelonia.fold :as fold]
         '[chelonia.rt] '[clojure.string :as str] '[clojure.set :as set] '[clojure.java.io :as io])

(def log (System/getenv "CHELONIA_LOG"))
(when (or (nil? log) (not (.exists (io/file log))))
  (println "cnf_lifecycle_test: skipped — set CHELONIA_LOG") (System/exit 0))
...
;; --- the life-os lifecycle, AS RULES (lifecycle DERIVED from the explicit
